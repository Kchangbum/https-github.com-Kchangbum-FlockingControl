...
%%
% % 1. 데이터 추출 (3x1x5001 구조 대응)
% t = out.u_acs.Time;
% 
% % --- ACS 데이터 처리 ---
% raw_acs = out.u_acs.Data;
% u_acs_vec = squeeze(raw_acs);      % 3 x 5001 로 압축
% mag_acs = sqrt(u_acs_vec(1,:).^2 + u_acs_vec(2,:).^2); % 크기 계산 (1 x 5001)
% mag_acs = mag_acs';                % t와 맞추기 위해 5001 x 1 로 변환
% 
% % --- PF 데이터 처리 ---
% raw_pf = out.u_pf.Data;
% u_pf_vec = squeeze(raw_pf);        % 3 x 5001 로 압축
% mag_pf = sqrt(u_pf_vec(1,:).^2 + u_pf_vec(2,:).^2);    % 크기 계산 (1 x 5001)
% mag_pf = mag_pf';                  % t와 맞추기 위해 5001 x 1 로 변환
% 
% %% Figure 8: 2. 통합 그래프 생성
% figure('Color', 'w', 'Name', 'Control Effort Analysis (ACS vs PF)');
% 
% % --- (Subplot 1) ACS 제어 입력 ---
% subplot(2, 1, 1);
% plot(t, mag_acs, 'b-', 'LineWidth', 1.5);
% grid on; grid minor;
% ylabel('||u_{ACS}|| [N]');
% title('ACS Control Effort (Formation & Alignment)');
% % Y축 범위를 데이터에 맞게 자동 조절하되 가독성을 위해 상단 여유 확보
% ylim([0 max(mag_acs)*1.2]); 
% 
% % --- (Subplot 2) PF 제어 입력 ---
% subplot(2, 1, 2);
% plot(t, mag_pf, 'r-', 'LineWidth', 1.5);
% grid on; grid minor;
% ylabel('||u_{PF}|| [N]');
% xlabel('Time [s]');
% title('Potential Field Effort (Collision Avoidance)');
% % PF는 초기에만 크고 나중에는 작아지는 특성을 확인하기 위함
% ylim([0 max(mag_pf)*1.2]);


%%
% 목표 원형 궤도 설정 (본인의 시뮬레이션 값으로 수정하세요)
% R_goal = 500;           % 목표 반지름 [m]
% Center_goal = [0, 0];   % 목표 중심 [North, East]
% % 2. 각 기체 및 군집 중심의 '목표 중심으로부터의 거리' 계산
% dist_1_g = sqrt((x1 - Center_goal(1)).^2 + (y1 - Center_goal(2)).^2);
% dist_2_g = sqrt((x2 - Center_goal(1)).^2 + (y2 - Center_goal(2)).^2);
% dist_3_g = sqrt((x3 - Center_goal(1)).^2 + (y3 - Center_goal(2)).^2);
% dist_4_g = sqrt((x4 - Center_goal(1)).^2 + (y4 - Center_goal(2)).^2);
% dist_c_g = sqrt((xc - Center_goal(1)).^2 + (yc - Center_goal(2)).^2);
% % 3. 오차 계산 (목표 반지름 500m 대비 얼마나 벗어났나?)
% err_1 = dist_1_g - R_goal;
% err_2 = dist_2_g - R_goal;
% err_3 = dist_3_g - R_goal;
% err_4 = dist_4_g - R_goal;
%% 그래프 시각화
% figure('Color', 'w', 'Name', '2D Orbital Tracking Analysis');
% subplot(2,1,1); % 첫 번째 그래프: 개별 기체의 궤도 이탈 정도
% hold on;
% plot(t, err_1, 'b', 'DisplayName', 'UAV 1 Error');
% plot(t, err_2, 'g', 'DisplayName', 'UAV 2 Error');
% plot(t, err_3, 'r', 'DisplayName', 'UAV 3 Error');
% plot(t, err_4, 'c', 'DisplayName', 'UAV 4 Error');
% yline(0, 'k--', 'Target Radius', 'LineWidth', 1.5);
% grid on; ylabel('Radial Error [m]');
% title('Individual UAV Deviation from Target Radius');
% legend('Location', 'northeastoutside');
% subplot(2,1,2); % 두 번째 그래프: 군집 중심의 편심(Center Offset)
% plot(t, dist_c_g, 'm', 'LineWidth', 2, 'DisplayName', 'Center Offset');
% grid on; ylabel('Distance [m]'); xlabel('Time [s]');
% title('Distance between Swarm Center and Orbit Center (Eccentricity)');
% legend('Location', 'northeastoutside');